...
let mmail=document.querySelector("#mmail")
...
let nameListner=mname.addEventListener('input',()=>{  
    if(mname.value.match(regname) != mname.value )
    {   
    ...
    let emailListner=mmail.addEventListener('input',()=>{
    if(mmail.value.match(regmail) != mmail.value)
    {   
    ...
    let textListner=message.addEventListener('input',()=>{
        ...
        else if(message.value.length > 400)
        {
            ...
            error2.textContent=message.value.length+"/400 ";
            check[2]=1
        }
        })
    ...
    $("#contactForm").submit((e)=>{
       ...
       {    value=false;
            $.ajax({
            url:"https://script.google.com/macros/s/AKfycbzekIdHZ2O2te88iJWKoz8mNx-I903tioWqeFQK45lrHxpkwPuofO61FLYiAtUwpZylbA/exec",
